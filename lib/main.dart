...
import 'package:hajeri/face_detection/main.dart';
// import 'package:hajeri/Pages/notification_message.dart';
...
import './face_detection/Pages/sign_up.dart' as faceScan;
...
Future<void> _firebaseMessagingForegroundHandler(RemoteMessage message) async {
  ...
  dev.log("Message data screen: ${message.data['screen']}");
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      routes: {
        // '/notification': (_) => NotificationsPage(),
...
class _HomeState extends State<Home> {
  ...
  // Future serviceStatusCheck() async {
  //   log("entered in post method::");
  //   try {
  //     var res = await http.get(Uri.parse("$kQRPointList"));
  ...
  //       for (var s in data) {
  //         Service service = Service(s['entolled'], s["ServiceStatus"]);

  //         prefs.setString("key", service.toString());
  ...
  getServiceStatus() async {
    var response =
        await http.get(Uri.parse('$kServiceList${prefs.getString('mobile')}'));
    if (response.statusCode == 200) {
  ...
  void initState() {
    ...
    FirebaseMessaging.onMessageOpenedApp.listen(notificationShowMethod());
...
////////*********************************************************************** */
    ...
    // if (initailMessage.data['screen'] == 'NotificationsPage') {
    //   log("In Initstate notification");
  ...
  notificationShowMethod() {
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async {
      dev.log("onMessageOpnedApp: ${message.data['screen']}");

      if (message.data["screen"] == "NotificationsPage") {
        setState(() {
          fromNotification = true;
        });
        dev.log("true");

        // var _id = int.tryParse(message.data["id"]) ?? 0;
            ...
            builder: (BuildContext context) {
              return Container(
                child: AlertDialog(
                  ...
                  content: Text("${message.data['click_action']}"),
                ),
              );
            });
        // navigatorKey.currentState
...
class Service {
  ...
  Service(this.status, this.serviceName);
}
